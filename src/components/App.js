...
function App() {
  ...
  const filteredCities = selectedState
    ? states.find((state) => state.name === selectedState).city
    : []

  const filteredLandmarks = selectedCity
    ? filteredCities.find((city) => city.name === selectedCity).landmarks
    : []
  ...
  return (
    <div id="main">
      ...
      <div>
        {selectedLandmark && (
          <div>
            <h2>{selectedLandmark}</h2>
            <p>
              {
                filteredLandmarks.find(
                  (landmark) => landmark.name === selectedLandmark
                ).description
              }
            </p>
          </div>
